...
const CancleOrder = ({ startDate, endDate }) => {
    const [cancle, setCancle] = useState([]);

    useEffect(() => {
        const fetchCanceledOrders = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/api/cancle?startDate=${startDate}&endDate=${endDate}`);
                setCancle(response.data);
            } catch (error) {
    ...
    const TableBody = () => (
        CanceledArr.items.map((item, index) => (  // เพิ่มพารามิเตอร์ index ที่นี่
            <Fragment key={item.id}>
                <View style={{ width: '100%', flexDirection: 'row' }}>
                    <View style={styles.tbody}>
                        <Text>{index + 1}</Text> {/* ใช้ index ที่ได้จากการ map เพื่อแสดงเลขลำดับ */}
                    </View>
                    <View style={styles.tbody}>
                    ...
                    <View style={styles.tbody}>
                        <Text>{item.PricePerUnit.toFixed(2)}</Text>
                    </View>
                    {/* <View style={styles.tbody}>
                        <Text>{(item.PricePerUnit*0.07).toFixed(2)}</Text>
                    </View> */}
                    <View style={styles.tbody}>
                        <Text>{(item.TotalQty)}</Text>
                    </View>
                    <View style={styles.tbody}>
                        <Text>{((item.PricePerUnit* 1.07) * item.TotalQty).toFixed(2)}</Text> 
                    </View>
                </View>
            </Fragment>
        ))
    ...
    const TableTotal = () => (
        <View style={{ width:'100%', flexDirection :'row'}}>
            ...
            <View style={styles.tbody}>
                <Text>{CanceledArr.items.reduce((sum, item)=> sum + (item.TotalQty), 0)}</Text>   
            </View>
            <View style={styles.tbody}>
                <Text>
                    {CanceledArr.items.reduce((sum, item)=> sum + ((item.PricePerUnit *1.07)*item.TotalQty), 0).toFixed(2)}
                </Text>  
            </View>
